...
class OneWaySystemController extends BasePopupController {
  Rx<OneWaySystem> model = OneWaySystem().obs;

  static String endPoint = 'one-way-system';
  ...
  Future getData() async {
    ...
    if (!isConnectInternet) {
      ...
      final response = await repository.getOneWaySystem(
          ticketController.ticketID, popupsDataModel.equipmentId, endPoint);

      if (response.isLoadSuccess) {
        model.value = response?.data ?? OneWaySystem();
  ...
  Future updateData() async {
    ...
    if (!isConnectInternet) {
      ...
      final params = {
        ...
        'oneWaySystem': model.value.toJson()
      };
  ...
  Future<void> getOffline(String ticketId) async {
    ...
    model.value = OneWaySystem.fromJson(data['oneWaySystem']);
    model.value.title = popupsDataModel.getPopupName();
  }

  void createDefaultIfNeeded() {
    // model.value.operatingStatus ??= TKTOptions.first.value;
    // model.value.problems ??= KCOptions.first.value;
    // model.value.handlingImmediatelyInspection ??= CKOptions.first.value;
  }
}

class OneWaySystemScreen extends BasePopupWidget {
  OneWaySystemScreen({PopupsDataModel popupsDataModel}) {
    _controller.popupsDataModel = popupsDataModel;
    Future.delayed(const Duration(milliseconds: 200), _controller.getData);
  }

  final OneWaySystemController _controller = OneWaySystemController();
  static String endPoint = 'one-way-system';
      ...
      child: Obx(() => Column(
            children: [
              ...
              ETextField(
                ...
                isRequire: true,
                enable: _controller.popupsDataModel.isAllowEdit,
                value: _controller.model.value.resistorInsulation,
                horizontalPadding: 0,
                onChange: (value) {
                  _controller.model.value.resistorInsulation = value;
                },
              ),
              ETextArea(
              ...
              EDropDown(
                ...
                images: _controller.model.value.images,
                defaultValue: _controller.model.value.problems,
                onChange: (option, mess) {
                  _controller.model.value.problems = option.value;
                  _controller.model.value.setUnusually(mess);
                },
                onAttachImages: (images, index) {
                  _controller.model.value.setImages(images, index);
