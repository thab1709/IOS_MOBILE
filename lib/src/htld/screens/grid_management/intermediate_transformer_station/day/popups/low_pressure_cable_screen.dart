...
class LowPressureCableController extends BasePopupController {
  Rx<LowPressureCable> model = LowPressureCable().obs;

  static String endPoint = 'low-pressure-cable';
  ...
  Future getData() async {
    ...
    if (!isConnectInternet) {
      ...
      final response = await repository.getPressureCable(
          ticketController.ticketID, popupsDataModel.equipmentId, endPoint);

      if (response.isLoadSuccess) {
        model.value = response?.data ?? LowPressureCable();
  ...
  Future updateData() async {
    ...
    if (!isConnectInternet) {
      ...
      final params = {
        ...
        'lowPressureCable': model.value.toJson()
      };
  ...
  Future<void> getOffline(String ticketId) async {
    ...
    model.value = LowPressureCable.fromJson(data['lowPressureCable']);
    model.value.title = popupsDataModel.getPopupName();
  }
...
class LowPressureCableScreen extends BasePopupWidget {
  LowPressureCableScreen({PopupsDataModel popupsDataModel}) {
    _controller.popupsDataModel = popupsDataModel;
    Future.delayed(const Duration(milliseconds: 200), _controller.getData);
  }

  final LowPressureCableController _controller = LowPressureCableController();
  static String endPoint = 'low-pressure-cable';
