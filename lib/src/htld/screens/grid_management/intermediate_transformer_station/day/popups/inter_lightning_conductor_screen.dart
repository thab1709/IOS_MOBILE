...
class InterLightningConductorScreen extends BasePopupWidget {
  InterLightningConductorScreen({@required PopupsDataModel popupsDataModel}) {
    _controller.popupsDataModel = popupsDataModel;
    Future.delayed(const Duration(milliseconds: 200), _controller.getData);
  }

  final _InterLightningConductorController _controller =
      _InterLightningConductorController();
...
class _InterLightningConductorController extends BasePopupController {
  Rx<InterLightningConductor> model = InterLightningConductor().obs;
  ...
  void createDefaultIfNeeded() {
    ...
    // model.value.grounding ??= TKTOptions.first.value;
  ...
  Future getData() async {
    ...
    if (!isConnectInternet) {
      ...
      final response = await repository.getInterLightningConductor(
          ticketController.ticketID, popupsDataModel.equipmentId, endPoint);

      if (response.isLoadSuccess) {
        model.value = response?.data ?? InterLightningConductor();
  ...
  Future updateData() async {
    ...
    if (!isConnectInternet) {
      ...
      final params = {
        ...
        'lightningConductor': model.toJson()
      };
  ...
  Future<void> getOffline(String ticketId) async {
    ...
    model.value = InterLightningConductor.fromJson(data['lightningConductor']);
    model.value.title = popupsDataModel.getPopupName();
  }
}
