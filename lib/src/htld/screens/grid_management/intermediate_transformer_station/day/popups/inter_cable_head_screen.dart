...
class InterCableHeadScreen extends BasePopupWidget {
  InterCableHeadScreen({@required PopupsDataModel popupsDataModel}) {
    _controller.popupsDataModel = popupsDataModel;
    Future.delayed(const Duration(milliseconds: 200), _controller.getData);
  }

  static String endPoint = 'cable-head';
  final _InterCableHeadController _controller = _InterCableHeadController();
...
class _InterCableHeadController extends BasePopupController {
  Rx<InterCableHead> model = InterCableHead().obs;
  ...
  Future getData() async {
    ...
    if (!isConnectInternet) {
      ...
      final response = await repository.getInterCableHead(
          ticketController.ticketID, popupsDataModel.equipmentId, endPoint);

      if (response.isLoadSuccess) {
        model.value = response?.data ?? InterCableHead();
  ...
  Future<void> getOffline(String ticketId) async {
    ...
    model.value = InterCableHead.fromJson(data['cableHead']);
    model.value.title = popupsDataModel.getPopupName();
  }
}
