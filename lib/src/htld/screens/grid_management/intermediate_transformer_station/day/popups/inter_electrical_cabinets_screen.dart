...
class InterElectricalCabinetsScreen extends BasePopupWidget {
  InterElectricalCabinetsScreen({@required PopupsDataModel popupsDataModel}) {
    _controller.popupsDataModel = popupsDataModel;
    Future.delayed(const Duration(milliseconds: 200), _controller.getData);
  }

  static String endPoint = 'electric-cabinet';
  final _InterElectricalCabinetsController _controller =
      _InterElectricalCabinetsController();
...
class _InterElectricalCabinetsController extends BasePopupController {
  Rx<InterElectricCabinet> model = InterElectricCabinet().obs;

  void createDefaultIfNeeded() {
    // model.value.condition ??= TRSHHOptions.first.value;
  ...
  String endPoint = 'electric-cabinet';
  ...
  Future getData() async {
    ...
    if (!isConnectInternet) {
      ...
      final response = await repository.getInterElectricCabinet(
          ticketController.ticketID, popupsDataModel.equipmentId, endPoint);

      if (response.isLoadSuccess) {
        model.value = response?.data ?? InterElectricCabinet();
  ...
  Future<void> getOffline(String ticketId) async {
    ...
    model.value = InterElectricCabinet.fromJson(data['electricCabinet']);
    model.value.title = popupsDataModel.getPopupName();
  }
}
