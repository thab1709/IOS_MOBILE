...
class FillingCabinetController extends BasePopupController {
  Rx<FillingCabinet> model = FillingCabinet().obs;

  String endPoint = 'filling-cabinet';
  ...
  Future getData() async {
    ...
    if (!isConnectInternet) {
      ...
      final response = await repository.getFillingCabinet(
          ticketController.ticketID, popupsDataModel.equipmentId, endPoint);

      if (response.isLoadSuccess) {
        model.value = response?.data ?? FillingCabinet();
  ...
  Future updateData() async {
    ...
    if (!isConnectInternet) {
      ...
      final params = {
        ...
        'fillingCabinet': model.value.toJson()
      };
  ...
  Future<void> getOffline(String ticketId) async {
    ...
    model.value = FillingCabinet.fromJson(data['fillingCabinet']);
    model.value.title = popupsDataModel.getPopupName();
  }
...
class FillingCabinetScreen extends BasePopupWidget {
  FillingCabinetScreen({PopupsDataModel popupsDataModel}) {
    _controller.popupsDataModel = popupsDataModel;
    Future.delayed(const Duration(milliseconds: 200), _controller.getData);
  }

  static String endPoint = 'filling-cabinet';
  final FillingCabinetController _controller = FillingCabinetController();
    ...
    return SingleChildScrollView(
      child: Obx(() => Column(
            children: [
              ...
              //   index: 2,
              //   images: _fillingCabinet.images,
              //   defaultValue: _fillingCabinet.operatingStatus,
              //   onChange: (option, mess) {
              //     _fillingCabinet.operatingStatus = option.value;
              //   },
              //   onAttachImages: (images, index) {
              //     _fillingCabinet.setImages(images, 2);
              //   },
              // ),
