...
class InterMeasuringSystemScreen extends BasePopupWidget {
  InterMeasuringSystemScreen({@required PopupsDataModel popupsDataModel}) {
    _controller.popupsDataModel = popupsDataModel;
    Future.delayed(const Duration(milliseconds: 200), _controller.getData);
  }

  final _InterMeasuringSystemController _controller =
      _InterMeasuringSystemController();
  static String endPoint = 'measuring-system';
    ...
    return SingleChildScrollView(
      child: Obx(() => Column(
            children: [
              ...
              EDropDown(NVTDNAOptions,
                  title: 'Tình trạng nguyên vẹn của hệ thống đo đếm',
                  index: 1,
                  enable: _controller.popupsDataModel.isAllowEdit,
...
class _InterMeasuringSystemController extends BasePopupController {
  Rx<InterMeasuringSystem> model = InterMeasuringSystem().obs;

  void createDefaultIfNeeded() {
    // model.value.meteringSystem ??= NVTDOptions.first.value;
    // model.value.workingMeter ??= BTKBTHHOptions.first.value;
  ...
  Future getData() async {
    ...
    if (!isConnectInternet) {
      ...
      final response = await repository.getInterMeasuringSystem(
          ticketController.ticketID, popupsDataModel.equipmentId, endPoint);

      if (response.isLoadSuccess) {
        model.value = response?.data ?? InterMeasuringSystem();
  ...
  Future<void> getOffline(String ticketId) async {
    ...
    model.value = InterMeasuringSystem.fromJson(data['measuringSystem']);
    model.value.title = popupsDataModel.getPopupName();
  }
}
