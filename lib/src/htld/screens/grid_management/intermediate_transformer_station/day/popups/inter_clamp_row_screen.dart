...
class InterClampRowScreen extends BasePopupWidget {
  InterClampRowScreen({@required PopupsDataModel popupsDataModel}) {
    _controller.popupsDataModel = popupsDataModel;
    Future.delayed(const Duration(milliseconds: 200), _controller.getData);
  }

  static String endPoint = 'clamp-row';
  final _InterClampRowController _controller = _InterClampRowController();
      ...
      child: Obx(() => Column(
            children: [
              ...
              EDropDown(BTNAOptions,
                  title:
                      'Tương ứng giữa tín hiệu đèn hoặc vị trí chỉ thị của các trang thiết bị đóng cắt, các khoá chuyển mạch với trạng thái của thiết bị nhất thứ',
                  index: 2,
                  defaultValue: _controller.model.value.lightSignal,
                  enable: _controller.popupsDataModel.isAllowEdit,
                  onSelectedAbnormalOption: (categoryName, abnormal, index) {
                    _controller.setAbnormal(
                        categoryName, _controller.model.value, abnormal, index);
                  ...
                  images: _controller.model.value.images,
                  onChange: (option, mess) {
                    _controller.model.value.lightSignal = option.value;
                    _controller.model.value.setUnusually(mess);
                  },
                  onAttachImages: (images, index) {
                    _controller.model.value.setImages(images, index);
...
class _InterClampRowController extends BasePopupController {
  Rx<InterClampRow> model = InterClampRow().obs;

  void createDefaultIfNeeded() {
    // model.value.condition ??= TKTOptions.first.value;
    // model.value.lightSignal ??= BTOptions.first.value;
  ...
  Future getData() async {
    ...
    if (!isConnectInternet) {
      ...
      final response = await repository.getInterClampRow(
          ticketController.ticketID, endPoint);

      if (response.isLoadSuccess) {
        model.value = response?.data ?? InterClampRow();
  ...
  Future updateData() async {
    ...
    if (!isConnectInternet) {
      ...
      final params = {'clampRow': model.toJson()};
  ...
  Future<void> getOffline(String ticketId) async {
    ...
    model.value = InterClampRow.fromJson(data['clampRow']);
    model.value.title = popupsDataModel.getPopupName();
  }
}
