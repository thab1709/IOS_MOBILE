...
class InterJointController extends BasePopupController {
  Rx<InterJoint> model = InterJoint().obs;
  ...
  Future getData() async {
    ...
    if (!isConnectInternet) {
      ...
      final response =
          await repository.getInterJoint(ticketController.ticketID, endPoint);

      if (response.isLoadSuccess) {
        model.value = response?.data ?? InterJoint();
  ...
  Future updateData() async {
    ...
    if (!isConnectInternet) {
      ...
      final params = {'joint': model.toJson()};
  ...
  Future<void> getOffline(String ticketId) async {
    ...
    model.value = InterJoint.fromJson(data['joint']);
    model.value.title = popupsDataModel.getPopupName();
  }
}

class InterJointScreen extends BasePopupWidget {
  InterJointScreen({@required PopupsDataModel popupsDataModel}) {
    _controller.popupsDataModel = popupsDataModel;
    Future.delayed(const Duration(milliseconds: 200), _controller.getData);
  }

  final InterJointController _controller = InterJointController();
  static String endPoint = 'joint';
