...
class ACSystemController extends BasePopupController {
  Rx<AlternatingCurrentSystem> model = AlternatingCurrentSystem().obs;

  String endPoint = 'alternating-current-system';
  ...
  Future getData() async {
    ...
    if (isConnectInternet) {
      ...
      final response = await repository.getAlternatingCurrentSystem(
          ticketController.ticketID, popupsDataModel.equipmentId, endPoint);

      if (response.isLoadSuccess) {
        model.value = response?.data ?? AlternatingCurrentSystem();
  ...
  Future updateData() async {
    ...
    if (!isConnectInternet) {
      ...
      final params = {
        ...
        'alternatingCurrentSystem': model.value.toJson()
      };
  ...
  Future<void> getOffline(String ticketId) async {
    ...
    model.value =
        AlternatingCurrentSystem.fromJson(data['alternatingCurrentSystem']);
    model.value.title = popupsDataModel.getPopupName();
  }
...
class ACSystemScreen extends BasePopupWidget {
  ACSystemScreen({PopupsDataModel popupsDataModel}) {
    _controller.popupsDataModel = popupsDataModel;
    Future.delayed(const Duration(milliseconds: 200), _controller.getData);
  }

  static String endPoint = 'alternating-current-system';
  final ACSystemController _controller = ACSystemController();
