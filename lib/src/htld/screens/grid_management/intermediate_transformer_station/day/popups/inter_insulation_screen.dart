...
class InterInsulationScreen extends BasePopupWidget {
  InterInsulationScreen({@required PopupsDataModel popupsDataModel}) {
    _controller.popupsDataModel = popupsDataModel;
    Future.delayed(const Duration(milliseconds: 200), _controller.getData);
  }

  final _InterInsulationController _controller = _InterInsulationController();
    ...
    return SingleChildScrollView(
      child: Obx(() => Column(
            children: [
              ...
              EDropDown(BTNVGNNTRCBLUKCDOptions,
                  title: 'Tình trạng',
                  index: 1,
                  enable: _controller.popupsDataModel.isAllowEdit,
              ...
              EDropDown(BTGRCNAOption,
                  title: 'Phụ kiện của cách điện',
                  index: 3,
                  enable: _controller.popupsDataModel.isAllowEdit,
                  defaultValue: _controller.model.value.insulationAccessories,
                  onSelectedAbnormalOption: (categoryName, abnormal, index) {
                    _controller.setAbnormal(
                        categoryName, _controller.model.value, abnormal, index);
                  },
                  images: _controller.model.value.images,
                  onChange: (option, mess) {
                    _controller.model.value.insulationAccessories =
                        option.value;
                    _controller.model.value.setUnusually(mess);
                  },
                  onAttachImages: (images, index) {
...
class _InterInsulationController extends BasePopupController {
  Rx<InterInsulation> model = InterInsulation().obs;

  void createDefaultIfNeeded() {
    // model.value.condition ??= BTNVGNNTRCBLUKCDOptions.first.value;
    // model.value.sound ??= BTRDOptions.first.value;
    // model.value.insulationAccessories ??= BTGRCOption.first.value;
  ...
  Future getData() async {
    ...
    if (!isConnectInternet) {
      ...
      final response = await repository.getInterInsulation(
          ticketController.ticketID, popupsDataModel.equipmentId, endPoint);

      if (response.isLoadSuccess) {
        model.value = response?.data ?? InterInsulation();
  ...
  Future<void> getOffline(String ticketId) async {
    ...
    model.value = InterInsulation.fromJson(data['insulation']);
    model.value.title = popupsDataModel.getPopupName();
  }
}
