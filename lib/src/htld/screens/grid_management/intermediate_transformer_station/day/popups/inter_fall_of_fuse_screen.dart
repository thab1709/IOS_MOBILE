...
class InterFallOfFuseScreen extends BasePopupWidget {
  InterFallOfFuseScreen({@required PopupsDataModel popupsDataModel}) {
    _controller.popupsDataModel = popupsDataModel;
    Future.delayed(const Duration(milliseconds: 200), _controller.getData);
  }

  static String endPoint = 'fall-of-fuse';
  final _InterFallOfFuseController _controller = _InterFallOfFuseController();
    ...
    return SingleChildScrollView(
      child: Obx(() => Column(
            children: [
              ...
              EDropDown(
                BTPTLDNAOptions,
                title: 'Tiếp điểm FCO và LBFCO',
                index: 5,
                enable: _controller.popupsDataModel.isAllowEdit,
                defaultValue: _controller.model.value.fcoContacts,
                onSelectedAbnormalOption: (categoryName, abnormal, index) {
                  _controller.setAbnormal(
                      categoryName, _controller.model.value, abnormal, index);
                },
                images: _controller.model.value.images,
                onChange: (option, mess) {
                  _controller.model.value.fcoContacts = option.value;
                  _controller.model.value.setUnusually(mess);
                },
                onAttachImages: (images, index) {
                  _controller.model.value.setImages(images, index);
              ...
              EDropDown(
                ...
                index: 7,
                enable: _controller.popupsDataModel.isAllowEdit,
                defaultValue: _controller.model.value.mountingLocation,
                onSelectedAbnormalOption: (categoryName, abnormal, index) {
                  _controller.setAbnormal(
                      categoryName, _controller.model.value, abnormal, index);
                },
                images: _controller.model.value.images,
                onChange: (option, mess) {
                  _controller.model.value.mountingLocation = option.value;
                  _controller.model.value.setUnusually(mess);
                },
                onAttachImages: (images, index) {
                  _controller.model.value.setImages(images, index);
...
class _InterFallOfFuseController extends BasePopupController {
  Rx<InterFallOfFuse> model = InterFallOfFuse().obs;
  ...
  void createDefaultIfNeeded() {
    ...
    // model.value.fuseHolder ??= BTCNPDOptions.first.value;
    // model.value.fcoContacts ??= BTPTLDOptions.first.value;
    // model.value.joint ??= BTOptions.first.value;
    // model.value.mountingLocation ??= PHOptions.first.value;
  ...
  Future getData() async {
    ...
    if (!isConnectInternet) {
      ...
      final response = await repository.getInterFallOfFuse(
          ticketController.ticketID, popupsDataModel.equipmentId, endPoint);

      if (response.isLoadSuccess) {
        model.value = response?.data ?? InterFallOfFuse();
  ...
  Future<void> getOffline(String ticketId) async {
    ...
    model.value = InterFallOfFuse.fromJson(data['fallofFuse']);
    model.value.title = popupsDataModel.getPopupName();
  }
}
