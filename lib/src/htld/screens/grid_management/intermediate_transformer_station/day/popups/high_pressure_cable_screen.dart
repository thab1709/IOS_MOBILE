...
class HighPressureCableController extends BasePopupController {
  Rx<HighPressureCable> model = HighPressureCable().obs;
  ...
  String endPoint = 'high-pressure-cable';
  ...
  Future getData() async {
    ...
    if (!isConnectInternet) {
      ...
      final response = await repository.getHighPressureCable(
          ticketController.ticketID, popupsDataModel.equipmentId, endPoint);

      if (response.isLoadSuccess) {
        model.value = response?.data ?? HighPressureCable();
  ...
  Future updateData() async {
    ...
    if (!isConnectInternet) {
      ...
      final params = {
        ...
        'highPressureCable': model.value.toJson()
      };
  ...
  Future<void> getOffline(String ticketId) async {
    ...
    model.value = HighPressureCable.fromJson(data['highPressureCable']);
    model.value.title = popupsDataModel.getPopupName();
  }

  void createDefaultIfNeeded() {
    // model.value.operatingStatus ??= TKTOptions.first.value;
    // model.value.pdStatus ??= TKTOptions.first.value;
    // model.value.possibleProblematic ??= KCOptions.first.value;
    // model.value.handlingImmediatelyInspection ??= CKOptions.first.value;
  }
}

class HighPressureCableScreen extends BasePopupWidget {
  HighPressureCableScreen({PopupsDataModel popupsDataModel}) {
    _controller.popupsDataModel = popupsDataModel;
    Future.delayed(const Duration(milliseconds: 200), _controller.getData);
  }

  static String endPoint = 'high-pressure-cable';
  final HighPressureCableController _controller = HighPressureCableController();
      ...
      child: Obx(() => Column(
            children: [
              ...
              EDropDown(
                ...
                images: _controller.model.value.images,
                defaultValue: _controller.model.value.pdStatus,
                onChange: (option, mess) {
                  _controller.model.value.pdStatus = option.value;
                },
                onAttachImages: (images, index) {
                  _controller.model.value.setImages(images, index);
                },
