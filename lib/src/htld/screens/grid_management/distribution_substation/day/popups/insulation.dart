...
class InsulationPopup extends BasePopupWidget {
  InsulationPopup({@required PopupsDataModel popupsDataModel}) {
    _controller.popupsDataModel = popupsDataModel;
    Future.delayed(const Duration(milliseconds: 200), _controller.getData);
  }

  static String endPoint = 'insulation';
  final _InsulationController _controller = _InsulationController();
      ...
      child: Obx(() => Column(
            ...
            children: [
              ...
              EDropDown(
                ...
                images: _controller.model.value.images,
                defaultValue: _controller.model.value.contactAtTerminals,
                onChange: (option, mess) {
                  _controller.model.value.contactAtTerminals = option.value;
                  _controller.model.value.setUnusually(mess);
                },
                onAttachImages: (images, index) {
                  _controller.model.value.setImages(images, index);
              ...
              EDropDown(
                ...
                images: _controller.model.value.images,
                defaultValue: _controller.model.value.accessoriesOfInsulation,
                onChange: (option, mess) {
                  _controller.model.value.accessoriesOfInsulation =
                      option.value;
                  _controller.model.value.setUnusually(mess);
                },
                onAttachImages: (images, index) {
...
class _InsulationController extends BasePopupController {
  Rx<InsulationModel> model = InsulationModel().obs;

  void createDefaultIfNeeded() {
    // model.value.insulation ??=BTNVCPBOptions.first.value;
    // model.value.contactAtTerminals ??=BTOptions.first.value;
    ...
    // model.value.accessoriesOfInsulation ??= BTGRCOption.first.value;
  }

  String endPoint = 'insulation';
  ...
  Future getData() async {
    ...
    if (!isConnectInternet) {
      ...
      final response = await repository.getInsulation(
          ticketController.ticketID, popupsDataModel.equipmentId, endPoint);

      if (response.isLoadSuccess) {
        model.value = response?.data ?? InsulationModel();
  ...
  Future<void> getOffine(String ticketId) async {
    ...
    model.value = InsulationModel.fromJson(data['insulation']);
    model.value.title = popupsDataModel.getPopupName();
  }
}
