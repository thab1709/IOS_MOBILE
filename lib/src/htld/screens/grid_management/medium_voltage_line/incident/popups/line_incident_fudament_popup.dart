...
class LineIncidentFudamentPopup extends BasePopupWidget {
  ...
  final _controller = LineFudamentController();
    ...
    return SingleChildScrollView(
      child: Obx(() => Column(
            ...
            children: [
              ...
              ELineDropDown(
                LineOption.DBVH_L_L_N_V_Option,
                title: 'Tình trạng',
                enable: _controller.isEnable(),
                index: 1,
...
class LineFudamentController extends BasePopupController {
  Rx<LineFundamentModel> model = LineFundamentModel().obs;

  String endPoint = 'fundament';
  ...
  Future getData() async {
    await super.getData();
    Future online() async {
      final response = await repository.getLineFundament(
      ...
      if (response.isLoadSuccess) {
        model.value = response?.data ?? LineFundamentModel();
    ...
    void offline() {
      final response = LocalDataManager.shared.getLinePopup<LineFundamentModel>(
          lineTicketController.ticketId,
          lineBranchInfo.id,
          popupsDataModel.equipmentId,
          popupsDataModel.inspectionCategory);
      model.value = response ?? LineFundamentModel();
      model.value.title = popupsDataModel.equipmentName;
      update();
    }
  ...
  Future updateData() async {
    ...
    Future offline({bool isOffline = true}) async {
      ...
      final result = await LocalDataManager.shared
          .saveLinePopup<LineFundamentModel>(
              model.value,
              lineTicketController.ticketId,
              lineBranchInfo.id,
              popupsDataModel.equipmentId,
