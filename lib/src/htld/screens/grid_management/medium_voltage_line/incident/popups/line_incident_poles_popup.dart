...
class LineIncidentPolesPopup extends BasePopupWidget {
...
class LinePoleIncidentController extends BasePopupController {
  Rx<LinePoleModel> model = LinePoleModel().obs;

  String endPoint = 'pole';
  ...
  Future getData() async {
    await super.getData();
    Future online() async {
      final response = await repository.getLinePole(
      ...
      if (response.isLoadSuccess) {
        model.value = response?.data ?? LinePoleModel();
    ...
    void offline() {
      final response = LocalDataManager.shared.getLinePopup<LinePoleModel>(
          lineTicketController.ticketId,
          lineBranchInfo.id,
          popupsDataModel.equipmentId,
          popupsDataModel.inspectionCategory);
      model.value = response ?? LinePoleModel();
      model.value.title = popupsDataModel.equipmentName;
      update();
    }
  ...
  Future updateData() async {
    ...
    Future offline({bool isOffline = true}) async {
      ...
      final result = await LocalDataManager.shared.saveLinePopup<LinePoleModel>(
          model.value,
          lineTicketController.ticketId,
          lineBranchInfo.id,
          popupsDataModel.equipmentId,
