...
class LineIncidentEarthingPopup extends BasePopupWidget {
  ...
  final _controller = LineEarthingController();
    ...
    return SingleChildScrollView(
      child: Obx(() => Column(
            ...
            children: [
              ...
              ELineDropDown(
                LineOption.DBVH_G_G_D_M_Option,
                title: 'Tình trạng',
                index: 1,
                enable: _controller.isEnable(),
              ...
              ELineDropDown(
                LineOption.DBVH_L_MHKDB_Option,
                title: 'Bu lông bắt tiếp địa',
                index: 2,
                enable: _controller.isEnable(),
                equipmentModel: equipmentModels,
                defaultValue: _controller.model.value
              ...
              ELineDropDown(
                LineOption.DBVH_NLKMD_Option,
                title: 'Cọc, thanh tiếp địa',
                index: 3,
                enable: _controller.isEnable(),
                equipmentModel: equipmentModels,
                defaultValue: _controller.model.value
              ...
              ETextField(
                ...
                enable: _controller.isEnable(),
                isRequire: true,
                value: _controller.model.value.resistor ??= "Không",
                onChange: (value) {
                  _controller.model.value.resistor = value;
                },
              ),
              ETextArea(
...
class LineEarthingController extends BasePopupController {
  Rx<LineEarthingModel> model = LineEarthingModel().obs;

  String endPoint = 'earthing';
  ...
  Future getData() async {
    await super.getData();
    Future online() async {
      final response = await repository.getLineEarthing(
      ...
      if (response.isLoadSuccess) {
        model.value = response?.data ?? LineEarthingModel();
    ...
    void offline() {
      final response = LocalDataManager.shared.getLinePopup<LineEarthingModel>(
          lineTicketController.ticketId,
          lineBranchInfo.id,
          popupsDataModel.equipmentId,
          popupsDataModel.inspectionCategory);
      model.value = response ?? LineEarthingModel();
      model.value.title = popupsDataModel.equipmentName;
      update();
    }
  ...
  Future updateData() async {
    ...
    Future offline({bool isOffline = true}) async {
      ...
      final result = await LocalDataManager.shared
          .saveLinePopup<LineEarthingModel>(
              model.value,
              lineTicketController.ticketId,
              lineBranchInfo.id,
              popupsDataModel.equipmentId,
