...
class LineIncidentCapacitorsPopup extends BasePopupWidget {
  ...
  final _controller = LineCapacitorsController();
    ...
    return SingleChildScrollView(
      child: Obx(() => Column(
            ...
            children: [
              ...
              ELineDropDown(
                LineOption.DBVH_TXKT_DNTDKT_TDG_G_Option,
                title: 'Tình trạng',
                index: 1,
                enable: _controller.isEnable(),
              ...
              ELineDropDown(
                ...
                equipmentModel: equipmentModels,
                defaultValue: _controller.model.value
                    .getProblemValue(5, LineContentOption.no),
                problemPositions: _controller.model.value?.problemPositions
...
class LineCapacitorsController extends BasePopupController {
  Rx<LineCapacitorModel> model = LineCapacitorModel().obs;

  String endPoint = 'capacitor';
  ...
  Future getData() async {
    await super.getData();
    Future online() async {
      final response = await repository.getLineCapacitor(
      ...
      if (response.isLoadSuccess) {
        model.value = response?.data ?? LineCapacitorModel();
    ...
    void offline() {
      final response = LocalDataManager.shared.getLinePopup<LineCapacitorModel>(
          lineTicketController.ticketId,
          lineBranchInfo.id,
          popupsDataModel.equipmentId,
          popupsDataModel.inspectionCategory);
      model.value = response ?? LineCapacitorModel();
      model.value.title = popupsDataModel.equipmentName;
      update();
    }
  ...
  Future updateData() async {
    ...
    Future offline({bool isOffline = true}) async {
      ...
      final result = await LocalDataManager.shared
          .saveLinePopup<LineCapacitorModel>(
              model.value,
              lineTicketController.ticketId,
              lineBranchInfo.id,
              popupsDataModel.equipmentId,
