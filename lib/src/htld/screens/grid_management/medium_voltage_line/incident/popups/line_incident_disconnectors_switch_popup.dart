...
class LineIncidentDisconnectorsSwitchPopup extends BasePopupWidget {
  ...
  final _controller = LineDisconnectorSwitchController();
    ...
    return SingleChildScrollView(
      child: Obx(() => Column(
            ...
            children: [
              ...
              ELineDropDown(
                LineOption.DBVH_G_CV_TXKT_LXTDG_Option,
                title: 'Lưỡi dao',
                index: 1,
                enable: _controller.isEnable(),
              ...
              ELineDropDown(
                ...
                equipmentModel: equipmentModels,
                defaultValue: _controller.model.value
                    .getProblemValue(2, LineContentOption.no),
                problemPositions: _controller.model.value?.problemPositions
...
class LineDisconnectorSwitchController extends BasePopupController {
  Rx<LineDisconnectorsSwitchModel> model = LineDisconnectorsSwitchModel().obs;

  String endPoint = 'disconnectors-switch';
  ...
  Future getData() async {
    await super.getData();
    Future online() async {
      final response = await repository.getLineDisconnectorsSwitch(
      ...
      if (response.isLoadSuccess) {
        model.value = response?.data ?? LineDisconnectorsSwitchModel();
    ...
    void offline() {
      final response = LocalDataManager.shared
          .getLinePopup<LineDisconnectorsSwitchModel>(
              lineTicketController.ticketId,
              lineBranchInfo.id,
              popupsDataModel.equipmentId,
              popupsDataModel.inspectionCategory);
      model.value = response ?? LineDisconnectorsSwitchModel();
      model.value.title = popupsDataModel.equipmentName;
      update();
    }
  ...
  Future updateData() async {
    ...
    Future offline({bool isOffline = true}) async {
      ...
      final result = await LocalDataManager.shared
          .saveLinePopup<LineDisconnectorsSwitchModel>(
              model.value,
              lineTicketController.ticketId,
              lineBranchInfo.id,
              popupsDataModel.equipmentId,
    ...
    Future online() async {
      final params = {
        ...
        'lineDisconnectorsSwitch': model.toJson()
      };
