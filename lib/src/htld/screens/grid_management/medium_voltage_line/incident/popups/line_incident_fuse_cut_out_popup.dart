...
class LineIncidentFuseCutOutPopup extends BasePopupWidget {
  ...
  final _controller = LineFuseCutOutController();
    ...
    return SingleChildScrollView(
      child: Obx(() => Column(
            ...
            children: [
              ...
              ELineDropDown(
                LineOption.DBVH_TXKT_DCL_CX_LXTDG_Option,
                title: 'Tình trạng',
                index: 1,
                enable: _controller.isEnable(),
...
class LineFuseCutOutController extends BasePopupController {
  Rx<LineFuseCutOutModel> model = LineFuseCutOutModel().obs;

  String endPoint = 'fuse-cut-out';
  ...
  Future getData() async {
    await super.getData();
    Future online() async {
      final response = await repository.getLineFuseCutOut(
          lineTicketController.ticketId,
          popupsDataModel.equipmentId,
          endPoint,
          lineBranchInfo.id);
      if (response.isLoadSuccess) {
        model.value = response?.data ?? LineFuseCutOutModel();
    ...
    void offline() {
      final response = LocalDataManager.shared
          .getLinePopup<LineFuseCutOutModel>(
              lineTicketController.ticketId,
              lineBranchInfo.id,
              popupsDataModel.equipmentId,
              popupsDataModel.inspectionCategory);
      model.value = response ?? LineFuseCutOutModel();
      model.value.title = popupsDataModel.equipmentName;
      update();
    }
  ...
  Future updateData() async {
    ...
    Future offline({bool isOffline = true}) async {
      ...
      final result = await LocalDataManager.shared
          .saveLinePopup<LineFuseCutOutModel>(
              model.value,
              lineTicketController.ticketId,
              lineBranchInfo.id,
              popupsDataModel.equipmentId,
