...
class LineIncidentInsulationPopup extends BasePopupWidget {
      ...
      child: Obx(() => Column(
            ...
            children: [
              ...
              ELineDropDown(
                ...
                equipmentModel: equipmentModels,
                defaultValue: _controller.model.value
                    .getProblemValue(3, LineContentOption.normal),
                problemPositions: _controller.model.value?.problemPositions
...
class LineInsulationController extends BasePopupController {
  Rx<LineInsulationModel> model = LineInsulationModel().obs;
  ...
  Future getData() async {
    await super.getData();
    Future online() async {
      final response = await repository.getLineInsulation(
      ...
      if (response.isLoadSuccess) {
        model.value = response?.data ?? LineInsulationModel();
    ...
    void offline() {
      final response = LocalDataManager.shared
          .getLinePopup<LineInsulationModel>(
              lineTicketController.ticketId,
              lineBranchInfo.id,
              popupsDataModel.equipmentId,
              popupsDataModel.inspectionCategory);
      model.value = response ?? LineInsulationModel();
      model.value.title = popupsDataModel.equipmentName;
      update();
    }
  ...
  Future updateData() async {
    ...
    Future offline({bool isOffline = true}) async {
      ...
      final result = await LocalDataManager.shared
          .saveLinePopup<LineInsulationModel>(
              model.value,
              lineTicketController.ticketId,
              lineBranchInfo.id,
              popupsDataModel.equipmentId,
