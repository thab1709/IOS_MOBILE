...
import 'package:evnmobile/src/htld/screens/grid_management/medium_voltage_line/incident/popups/line_incident_rod_rap_popup.dart';
...
class LineIncidentRodRapPopup extends BasePopupWidget {
  ...
  final _controller = LineRodRapController();
    ...
    return SingleChildScrollView(
      child: Obx(() => Column(
            ...
            children: [
              ...
              ELineDropDown(
                LineOption.DBVH_BT_NA_Option,
                title: 'Tình trạng mối nối, nối đất',
                index: 3,
                enable: _controller.isEnable(),
                equipmentModel: equipmentModels,
                defaultValue: _controller.model.value
                    .getProblemValue(3, LineContentOption.ensureOperation),
                problemPositions: _controller.model.value?.problemPositions
              ...
              ELineDropDown(
                LineOption.DTC_QX_QG_NA_Option,
                title: 'Khoảng cách hai mỏ phóng',
                index: 4,
                enable: _controller.isEnable(),
                equipmentModel: equipmentModels,
                defaultValue: _controller.model.value
                    .getProblemValue(4, LineContentOption.normal),
                problemPositions: _controller.model.value?.problemPositions
...
class LineRodRapController extends BasePopupController {
  Rx<LineRodGadModel> model = LineRodGadModel().obs;
  String endPoint = 'rod-gap';
  ...
  Future getData() async {
    await super.getData();
    Future online() async {
      final response = await repository.getLineRodGap(
      ...
      if (response.isLoadSuccess) {
        model.value = response?.data ?? LineRodGadModel();
    ...
    void offline() {
      final response = LocalDataManager.shared.getLinePopup<LineRodGadModel>(
          lineTicketController.ticketId,
          lineBranchInfo.id,
          popupsDataModel.equipmentId,
          popupsDataModel.inspectionCategory);
      model.value = response ?? LineRodGadModel();
      model.value.title = popupsDataModel.equipmentName;
      update();
    }
