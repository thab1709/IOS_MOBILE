...
class LineIncidentMeasureBoundariesPopup extends BasePopupWidget {
  ...
  final _controller = LineMeasureBoundariesController();
    ...
    return SingleChildScrollView(
      child: Obx(() => Column(
            ...
            children: [
              ...
              ELineDropDown(
                LineOption.DBVH_BT_Option,
                title: 'Hộp công tơ và các thiệt bị khác',
                index: 3,
                enable: _controller.isEnable(),
                equipmentModel: equipmentModels,
                defaultValue: _controller.model.value
...
class LineMeasureBoundariesController extends BasePopupController {
  Rx<LineMeasureTheBoundaryModel> model = LineMeasureTheBoundaryModel().obs;

  String endPoint = 'measure-boundary';
  ...
  Future getData() async {
    await super.getData();
    Future online() async {
      final response = await repository.getLineMeasureTheBoundary(
      ...
      if (response.isLoadSuccess) {
        model.value = response?.data ?? LineMeasureTheBoundaryModel();
    ...
    void offline() {
      final response = LocalDataManager.shared
          .getLinePopup<LineMeasureTheBoundaryModel>(
              lineTicketController.ticketId,
              lineBranchInfo.id,
              popupsDataModel.equipmentId,
              popupsDataModel.inspectionCategory);
      model.value = response ?? LineMeasureTheBoundaryModel();
      model.value.title = popupsDataModel.equipmentName;
      update();
    }
  ...
  Future updateData() async {
    ...
    Future offline({bool isOffline = true}) async {
      ...
      final result = await LocalDataManager.shared
          .saveLinePopup<LineMeasureTheBoundaryModel>(
              model.value,
              lineTicketController.ticketId,
              lineBranchInfo.id,
              popupsDataModel.equipmentId,
    ...
    Future online() async {
      final params = {
        ...
        'lineMeasureTheBoundary': model.toJson()
      };
