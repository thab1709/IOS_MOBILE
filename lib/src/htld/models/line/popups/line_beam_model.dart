...
class LineBeamModel extends PopupBaseModel {
  String materialClingingToBeam;

  LineBeamModel({
    this.materialClingingToBeam,
  });

  LineBeamModel.fromJson(JSON json) {
    materialClingingToBeam = json['materialClingingToBeam'].string;
  ...
  Map<String, dynamic> toJson() {
    ...
    map['materialClingingToBeam'] = materialClingingToBeam;
  ...
  bool validateData() {
    ...
    if (_ticketController.argument.ticketType != TicketType.incidentDay) {
      return ![specificPhenomena, materialClingingToBeam, suggestedHandlingOfAbnormal].contains(null) && listDistinct.length == 4;
    } else {
      return ![specificPhenomena, suggestedHandlingOfAbnormal].contains(null) &&
          listDistinct.length == 2;
    }
