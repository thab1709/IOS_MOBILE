...
class LineEarthingModel extends PopupBaseModel {
  String resistor;
  ...
  LineEarthingModel.fromJson(JSON json) {
    resistor = json['resistor'].string;
  ...
  bool validateData() {
    ...
    if (_ticketController.argument.ticketType != TicketType.incidentDay) {
      return ![specificPhenomena, suggestedHandlingOfAbnormal, resistor].contains(null) && listDistinct.length == 5;

    } else {
      return ![specificPhenomena, suggestedHandlingOfAbnormal].contains(null) && listDistinct.length == 4;
    }
  }
}
