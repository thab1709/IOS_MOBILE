...
class LinePoleModel extends PopupBaseModel {
  String materialClingToPole;

  LinePoleModel({
    this.materialClingToPole,
  });

  LinePoleModel.fromJson(JSON json) {
    materialClingToPole = json['materialClingToPole'].string;
  ...
  Map<String, dynamic> toJson() {
    ...
    map['materialClingToPole'] = materialClingToPole;
  ...
  bool validateData() {
    ...
    if (_ticketController.argument.ticketType != TicketType.incidentDay) {
      return ![specificPhenomena, suggestedHandlingOfAbnormal, materialClingToPole].contains(null) && listDistinct.length == 6;
    } else {
      return ![specificPhenomena, suggestedHandlingOfAbnormal].contains(null) && listDistinct.length == 2;
    }
