// @dart=2.9
import 'package:evnmobile/src/htdct/models/equipment_model.dart';
import 'package:flutter/material.dart';
import 'package:flutter_keyboard_visibility/flutter_keyboard_visibility.dart';
import 'package:get/get.dart';

import '../../../../../../common/constance/app_color.dart';
import '../../../../../../models/line/popups/csv_model.dart';
import '../../../../base/base_popup_wigget.dart';
import 'check_bonded/check_bonded_screen.dart';
import 'check_box_heat_emission/check_box_heat_emission_screen.dart';
import 'check_link_box/check_link_box_screen.dart';
import 'tunnel_cable_controller.dart';

class TunnelCableScreen extends BasePopupWidget {
  TunnelCableScreen(
      {@required EquipmentModel model,
      List<EquipmentModel> equipmentsDestination}) {
    _controller.equipmentModel = model;
    _controller.equipmentsDestination = equipmentsDestination;
    Future.delayed(const Duration(milliseconds: 200), () {
      if (equipmentsDestination != null) {
        _controller.copyData();
      } else {
        _controller.getData();
      }
    });
  }

  final _controller = TunnelCableController<CsvModel>();

  @override
  Widget build(BuildContext context) {
    return Obx(_renderBody);
  }

  Widget _renderBody() {
    final keyboardVisibilityController = KeyboardVisibilityController();
    keyboardVisibilityController.onChange.listen((visible) {
      if (!visible) {
        _controller.updateTem();
      }
    });
    return Column(
      children: [
        Visibility(visible: false, child: Text(_controller.invalid.toString())),
        Expanded(
          child: _buildContent(),
        ),
      ],
    );
  }

  Widget _buildContent() {
    return SingleChildScrollView(
      child: Container(
        color: HighElectricAppColor.appBackgroundColor,
        padding: const EdgeInsets.all(15),
        child: Column(
          children: [
            buildCommonInfoLine(_controller),
            if (_controller.transformerTicketController.checkAbnormalNotify(
                _controller.dataModel.value.checkBondedAbnormal,
                nonCheck: true, title: 1))
              buildCheckBondedScreen(_controller),
            if (_controller.transformerTicketController.checkAbnormalNotify(
                _controller.dataModel.value.cableBoxHeatEmission,
                nonCheck: true, title: 2))
              buildCableBoxHeatEmission(_controller),
            if (_controller.transformerTicketController.checkAbnormalNotify(
                _controller.dataModel.value.linkboxCableHeatEmission,
                nonCheck: true, title: 3))
              buildLinkBox(_controller),
          ],
        ),
      ),
    );
  }

  @override
  void saveData() {
    _controller.updateData();
  }
}

