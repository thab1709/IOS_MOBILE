...
class GroupCheckController extends GetxController {
  ...
  Future getGroupCheck() async {
    ...
    if (res.isLoadSuccess) {
      ...
      for (var position = 0; position < tbaGroupCheckModel.value.groups.length; position++) {
        futures.add(getImagesUser(position, tbaGroupCheckModel.value.groups[position].idImage));}
  ...
  Future getImagesUser(int position, String imagesId) async {
    final res = await _tbaRep.getImages(
        idImages: imagesId, testType: transformerTicketController.testType, isBackground: true);
    if (res.isLoadSuccess) {
