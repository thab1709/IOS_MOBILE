...
abstract class AssetPickerProvider<Asset, Path> extends ChangeNotifier {
  ...
  /// Select asset.
  /// 选中资源
  void selectAsset(BuildContext context, Asset item, String itemName) {
    if (selectedAssets.length == maxAssets || selectedAssets.contains(item)) {
      return;
    }
    if(imagesSelectedInPopup?.contains(itemName) == true){
      showDialog<void>(
        context: context,
...
class DefaultAssetPickerProvider
  ...
  Future<void> getAssetList() async {
    if (_pathEntityList.isNotEmpty) {
      ...
      totalAssetsCount = await currentPathEntity!.assetCountAsync;
  ...
  Future<void> loadMoreAssets() async {
    final List<AssetEntity> assets =
       ...
       page: currentAssetsListPage, size: pageSize,
    ))
            .toList();
  ...
  Future<Uint8List?> getFirstThumbFromPathEntity(
    ...
    final AssetEntity asset = (await pathEntity.getAssetListRange(
      start: 0,
        ...
        .elementAt(0);
    final Uint8List? assetData = await asset.thumbnailDataWithSize(ThumbnailSize(pathThumbSize, pathThumbSize));
    return assetData;
  }
}
